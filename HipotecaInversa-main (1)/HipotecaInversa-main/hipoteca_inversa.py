class ErrorValoresIngresadosPorcentajes (Exception):
    def __init__(self,porcentaje_precio_real) -> None:
        super().__init__(f"El valor de los porcentajes{porcentaje_precio_real} de la vivienda y la tasa de interes no pueden ser negativos.")

class ErrorEdadIncorrecta (Exception):
    def __init__(self,edad):
        super().__init__(f"El valor de la edad {edad} del cliente no es válida. La edad ingresada es negativa o menor a 65")

class ErrorCuotas (Exception):
    def __init__(self,total_cuotas):
        super().__init__(f"El valor de las cuotas que ingreso {total_cuotas} no son validas válido.")
...
def calcular_ingreso_mensual(edad, precio_de_la_vivienda, porcentaje_precio_real, total_cuotas):
    ...
    if porcentaje_precio_real < 0:
        raise ErrorValoresIngresadosPorcentajes(porcentaje_precio_real)
